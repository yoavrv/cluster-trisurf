...
ts_bool triangle_add_neighbour(ts_triangle *tria, ts_triangle *ntria){
    ...
    if (tria->neigh_no==3){
        fatal("attempt to add triangle neighbor beyond 3 in triangle_add_neighbor",100);
    }
    tria->neigh_no++;
...
ts_bool triangle_normal_vector(ts_triangle *tria){
    ...
    alpha = ( (x32*x32 + y32*y32 + z32*z32)*( x31*x21 + y31*y21 + z31*z21) )/(2.0*norm_sqr);
    beta  = ( (x31*x31 + y31*y31 + z31*z31)*(-x21*x32 - y21*y32 - z21*z32) )/(2.0*norm_sqr);
    gamma = ( (x21*x21 + y21*y21 + z21*z21)*( x31*x32 + y31*y32 + z31*z32) )/(2.0*norm_sqr);
...
ts_bool debug_triangle_normal_vector(ts_triangle *tria){
    ts_double x1,x2,x3,y1,y2,y3,z1,z2,z3;
    ts_double x21,x31,x32,y21,y31,y32,z21,z31,z32;
    ts_double xcross, ycross, zcross, norm_sqr, norm;
    ts_double alpha, beta, gamma;
